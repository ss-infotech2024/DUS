...
const slides = [
  ...
  },
  {
    ...
    image:
      "https://images.unsplash.com/photo-1518779578993-ec3579fee39f?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80",
    ctaText: "Get Started",
...
export function HeroSlider() {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden bg-gradient-to-br py-24 from-slate-900 via-blue-900 to-indigo-900">
      {/* Background Image */}
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            ...
            <div className="text-white space-y-6 sm:space-y-8 animate-fade-in">
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <p className="text-base sm:text-lg lg:text-2xl text-blue-100 max-w-xl sm:max-w-2xl leading-relaxed">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Button size="lg" className="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white border-0 text-lg px-6 sm:px-8 py-4 h-auto shadow-2xl shadow-blue-500/25">
                  <Play className="mr-2 h-5 w-5" />
                  {currentSlideData.ctaText}
                </Button>
                <Button size="lg" variant="outline" className="w-full sm:w-auto border-white/30 text-gray-200 hover:bg-white text-cyan-700 hover:text-black backdrop-blur-sm text-lg px-6 sm:px-8 py-4 h-auto">
              ...
              {/* Stats */}
              <div className="flex flex-row justify-between sm:justify-start sm:flex-row items-center sm:space-x-8 space-y-4 sm:space-y-0 pt-4">
                <div className="text-center">
                  <div className="text-2xl sm:text-3xl font-bold text-white">{currentSlideData.stats.value}</div>
                ...
                <div className="flex flex-wrap gap-2">
                  {technologies.slice(0, 8).map((tech) => (
                    <Badge
                      key={tech}
                      variant="secondary"
                      className="bg-white/10 backdrop-blur-sm border-white/20 text-white hover:bg-white/20 text-xs sm:text-sm px-2 sm:px-3 py-1"
                    >
                      {tech}
                    </Badge>
                  ))}
            ...
            {/* Right Feature Cards (hidden on mobile, visible from md up) */}
            <div className="hidden md:block space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 sm:p-6 border border-white/20 hover:bg-white/15 transition-all duration-300">
                  <Award className="h-8 w-8 sm:h-10 sm:w-10 text-yellow-400 mb-3 sm:mb-4" />
                  <h3 className="text-white font-semibold text-base sm:text-lg mb-1 sm:mb-2">Industry Recognition</h3>
                  <p className="text-blue-200 text-xs sm:text-sm">Award-winning development team</p>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 sm:p-6 border border-white/20 hover:bg-white/15 transition-all duration-300">
                  <TrendingUp className="h-8 w-8 sm:h-10 sm:w-10 text-green-400 mb-3 sm:mb-4" />
                  <h3 className="text-white font-semibold text-base sm:text-lg mb-1 sm:mb-2">Growth Focused</h3>
                  <p className="text-blue-200 text-xs sm:text-sm">Scalable solutions for your business</p>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 sm:p-6 border border-white/20 hover:bg-white/15 transition-all duration-300 sm:col-span-2">
                  <Code2 className="h-8 w-8 sm:h-10 sm:w-10 text-blue-400 mb-3 sm:mb-4" />
                  <h3 className="text-white font-semibold text-base sm:text-lg mb-1 sm:mb-2">Modern Tech Stack</h3>
      ...
      {/* Navigation Controls */}
      <div className="absolute  bottom-8 sm:bottom-8 left-1/2 transform -translate-x-1/2 z-20">
            ...
            {slides.map((_, index) => (
              <button
                ...
                className={`h-2 sm:h-3 rounded-full transition-all duration-300 ${
                  index === currentSlide ? 'bg-white w-5 sm:w-8' : 'bg-white/40 hover:bg-white/60 w-2 sm:w-3'
                }`}
                onMouseEnter={() => setIsAutoPlaying(false)}
                onMouseLeave={() => setIsAutoPlaying(true)}
              />
